    ...
    <div class="card-body">
      ...
      <form action="{{ url('blogs/'.$blog->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <div class="card border-primary">
                <div class="card-body">
                    ...
                    <input type="hidden" id="archivo_actual" name="archivo_actual" value="{{ $blog->archivo ?? '' }}">
                    @if($blog->tipo === 'imagen' && $blog->archivo)
                        <img class="img-thumbnail" id="img-preview" src="{{ asset($blog->archivo) }}" style="display: block;">
                    @elseif($blog->tipo === 'video' && $blog->archivo)
                        <video id="video-preview" controls style="display: block;">
        ...
        <div class="form-group">
          ...
          <input type="text" name="title" class="form-control" value="{{ old('title', $blog->titulo) }}">
        </div>
        <div class="form-group">
            <label for="description">Descripción</label>
            <textarea rows="4" name="description" class="form-control">{{ old('description', wrapBrTags($blog->descripcion)) }}</textarea>
        </div>
